...
class UNetSD_T2V_DEMO(nn.Module):
    def __init__(self,
            config=None,
            in_dim=4,
            dim=512,
            ...
            frame_num = 8,
            **kwargs):
        super(UNetSD_T2V_DEMO, self).__init__()
        ...
        for i, (in_dim, out_dim) in enumerate(zip(enc_dims[:-1], enc_dims[1:])):
            for j in range(num_res_blocks):
                ...
                if scale in attn_scales:
                    ...
                    block.append(
                            TemporalTransformer4Cross(
                                out_dim, out_dim // head_dim, head_dim, depth=1, context_dim=self.context_dim,
                                disable_self_attn=False, use_linear=True, multiply_zero=use_image_dataset
                            )
                    )
    ...
    def forward(self, 
        ...
        **kwargs):
        ...
        # [Temporal Context]
        temporal_context = x.new_zeros(batch, 0, self.context_dim)
        if temporal_y is not None:
            temporal_y_context = temporal_y
            temporal_context = torch.cat([temporal_context, temporal_y_context], dim=1)
        else:
            raise ValueError('temporal_y is required')
            # y_context = self.zero_y.repeat(batch, 1, 1)[:, :1, :]
            # context = torch.cat([context, y_context], dim=1)
        # original_context = context
        # temporal_context = temporal_context.repeat_interleave(repeats=f, dim=0)
        
        
        x = rearrange(x, 'b c f h w -> (b f) c h w')
        xs = []
        for block in self.input_blocks:
            x = self._forward_single(block, x, embeddings, context, time_rel_pos_bias, focus_present_mask, video_mask,temporal_context=temporal_context,conditional=kwargs['conditional'] if 'conditional' in kwargs else False,eot_idx=eot_idx,attention_store=attention_store)
            xs.append(x)
        ...
        for block in self.output_blocks:
            ...
            x = self._forward_single(block, x, embeddings, context, time_rel_pos_bias,focus_present_mask, video_mask, reference=xs[-1] if len(xs) > 0 else None,temporal_context=temporal_context,conditional=kwargs['conditional'] if 'conditional' in kwargs else False,eot_idx=eot_idx,attention_store=attention_store)
    ...
    def _forward_single(self, module, x, e, context, time_rel_pos_bias, focus_present_mask, video_mask, reference=None, temporal_context=None,conditional=False,eot_idx=None,attention_store=None):
        if isinstance(module, ResidualBlock):
            ...
            module = checkpoint_wrapper(module) if self.use_checkpoint else module
            x = module(x, context, conditional=conditional)
        elif isinstance(module, TemporalTransformer4Cross):
            module = checkpoint_wrapper(module) if self.use_checkpoint else module
            x = rearrange(x, '(b f) c h w -> b c f h w', b = self.batch)
            x = module(x, temporal_context,conditional=conditional,eot_idx=eot_idx,attention_store=attention_store,batch_size=self.batch)
            x = rearrange(x, 'b c f h w -> (b f) c h w', b = self.batch)
            ...
            for block in module:
                x = self._forward_single(block,  x, e, context, time_rel_pos_bias, focus_present_mask, video_mask, reference,temporal_context=temporal_context,conditional=conditional,eot_idx=eot_idx,attention_store=attention_store)
        else:
            x = module(x)
        return x
