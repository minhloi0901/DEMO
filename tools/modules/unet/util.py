...
class MemoryEfficientCrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None,conditional=False):
        q = self.to_q(x)
...
class MemoryEfficientCrossAttention4Cross(nn.Module):
    def __init__(self, query_dim, max_bs=4096, context_dim=None, heads=8, dim_head=64, dropout=0.0):
        super().__init__()
        inner_dim = dim_head * heads
        context_dim = default(context_dim, query_dim)
    ...
    def forward(self, x, context=None, mask=None,conditional=False,eot_idx=None,attention_store=None,batch_size=None):
        q = self.to_q(x)
        context = default(context, x)
        k = self.to_k(context)
        v = self.to_v(context)
        # if self.textrw is not None:
            # k = self.textrw(k,q)
            # v = self.textrw(v,q)
        # else:        
        k = k.repeat_interleave(q.shape[0]//context.shape[0],dim=0)
        v = v.repeat_interleave(q.shape[0]//context.shape[0],dim=0)
        ...
        if eot_idx is not None:
            ...
            attn = rearrange(attn, '(b h w m) f t -> b h w m f t', b=batch_size,h=h,w=w,m=self.heads)
            avg_attn = attn.mean(dim=3)
            eot_avg_attn = avg_attn[torch.arange(batch_size),:,:,:,eot_idx]
            attention_store.append(eot_avg_attn)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None,conditional=False):
        ...
        for i, block in enumerate(self.transformer_blocks):
            x = block(x, context=context[i],conditional=conditional)
        if self.use_linear:
            x = self.proj_out(x)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None,conditional=False):
        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None) + x
        x = self.attn2(self.norm2(x), context=context,conditional=conditional) + x
        x = self.ff(self.norm3(x)) + x
        return x
    
class BasicTransformerBlock4Cross(nn.Module):
    # ATTENTION_MODES = {
    #     "softmax": CrossAttention,  # vanilla attention
    #     "softmax-xformers": MemoryEfficientCrossAttention
    # }
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True,
        ...
        self.attn1 = attn_cls(query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout,
                              context_dim=context_dim if self.disable_self_attn else None)  # is a self-attention if not self.disable_self_attn
        self.ff = FeedForward(dim, dropout=dropout, glu=gated_ff)
        self.attn2 = MemoryEfficientCrossAttention4Cross(query_dim=dim, context_dim=context_dim,
                              heads=n_heads, dim_head=d_head, dropout=dropout)  # is self-attn if context is none
        self.norm1 = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim)
        self.norm3 = nn.LayerNorm(dim)
    ...
    def forward(self, x, context=None,conditional=False,eot_idx=None,attention_store=None,batch_size=None):
        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None) + x
        x = self.attn2(self.norm2(x), context=context,conditional=conditional,eot_idx=eot_idx,attention_store=attention_store,batch_size=batch_size) + x
        x = self.ff(self.norm3(x)) + x
        return x
...
class TemporalTransformer4Cross(nn.Module):
    def __init__(self, in_channels, n_heads, d_head,
                 depth=1, dropout=0., context_dim=None,
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock4Cross(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim[d],
                                   disable_self_attn=disable_self_attn, checkpoint=use_checkpoint)
             for d in range(depth)]
        )
        if not use_linear:
    ...
    def forward(self, x, context=None,conditional=False,eot_idx=None,attention_store=None,batch_size=None):
        ...
        x = rearrange(x, 'b f h w c-> (b h w) f c').contiguous()
        for i, block in enumerate(self.transformer_blocks):
            x = block(x, context=context[i],conditional=conditional,eot_idx=eot_idx,attention_store=attention_store,batch_size=batch_size)
            
        if self.use_linear:
            x = self.proj_out(x)
            x = rearrange(x, '(b h w) f c -> b c f h w', h=h, w=w).contiguous()
